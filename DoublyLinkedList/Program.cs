...
namespace DoublyLinkedList {
    class Program {
        static void Main(string[] args) {
            ...
            Log("\nTesting iterator");
            int[] itest = new int[] { 10, 15, 7, 5, 3, 1, 0, 4 };
            DLinkedList<int>.Iterator iter = test.Begin();
            int index = 0;
            do {
        ...
        public static void Test(DLinkedList<int> list, params int[] test) {
            bool error = false;

            if (list.Head != null) {
                if (list.Head.Data != test[0]) {
            ...
            if (!error && list.Tail != null) {
                if (list.Tail.Data != test[test.Length - 1]) {
                    PrintError(list, test);
                    error = true;
                }
            }
            else if (!error && test.Length != 0) {
                PrintError(list, test);
                error = true;
            }

            if (!error && list.Size != test.Length) {
                PrintError(list, test);
                error = true;
            }
            else if (!error) {
                int index = 0;
                DLinkedList<int>.Node node = list.Head;
        ...
        public static void PrintError(DLinkedList<int> list, params int[] test) {
        ...
        public static string AsString<T>(DLinkedList<T> list) {
            StringBuilder sb = new StringBuilder();
            sb.Append("Size: ");
            sb.Append(list.Size);
            sb.Append(", { ");
            DLinkedList<T>.Node node = list.Head;
